...
class MDSpec:
    ...
    def load(markdown_spec_dir: pathlib.Path) -> Optional[Specification]:  # pylint:disable=R0914
        ...
        curr_section = Section(
            heading4, "#".join([spec.spec_dir, heading4.rsplit(".", maxsplit=1)[-1]]), curr_line, curr_line
        )
        ...
        while curr_line < len(lines):
            ...
            if "#### " in line:
                ...
                curr_section = Section(
                    heading4, "#".join([spec.spec_dir, heading4.split(".")[-1]]), curr_line, curr_line
                )
                ...
                target = line.replace("### ", heading1 + ".").replace(" ", "-").removesuffix("\n").lower()
                heading3 = (
                    line.replace("### ", heading2 + ".").replace(" ", "-").removesuffix("\n").replace("/", "").lower()
                )
                # set the end line of the previous section
                curr_section.end_line = curr_line - 1
                # set the start line of the new section
                curr_section = Section(
                    heading3, "#".join([spec.spec_dir, heading3.split(".")[-1]]), curr_line, curr_line
                )
                # turn the new section to curr_section
                ...
                curr_section = Section(
                    heading2,
                    "#".join([str(markdown_spec_dir.resolve()), heading2.split(".")[-1]]),
                    curr_line,
                    curr_line,
                )
                # turn the new section to curr_section
