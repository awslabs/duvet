$(document).ready(function () {
    $.ajax({
        ...
        Object.keys(response.specifications).forEach((id) => {
            const spec = response.specifications[id];
            ...
            for (i = 0; i < sections.length; i++) {
                for (j = 0; j < sections[i].requirements.length; j++) {
                console.log(sections[i].requirements[j])
                tr = "<tr>";
                tr += "<td>";
                tr += sections[i].requirements[j].target_section;
                tr += "</td>";
                tr += "<td>";
                tr += sections[i].requirements[j].level;
                tr += "</td>";
                tr += "<td>";
                tr += sections[i].requirements[j].isComplete;
                tr += "</td>";
                tr += "<td>";
                tr += sections[i].requirements[j].comment;
                tr += "</td>";
                tr += "<tr>";
                $("#iam_tbody").append(tr);
            }
...
${quote}`;
        ...
        function mapLine(line) {
            if (typeof line === "string")
                return [{annotations: [], status: response.refs[0], text: line}];

            return line.map((ref) => {
                if (typeof ref === "string")
                    return {annotations: [], status: response.refs[0], text: ref};

                const [ids, status, text] = ref;
                const annotations = ids.map((id) => response.annotations[id]);
                return {
                    annotations,
                    status: response.refs[status] || response.refs[0],
                    text,
                };
            });
        }
