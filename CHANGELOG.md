...
* **Behavior change** — the `coverage` check now requires a single common witness for each test/implementation pair, and holds the test to *every* witness it binds: a pair passes only when at least one coverage source binds the test and all sources bound to it show the implementation executed (`design/witness/decisions.md`, Decisions 1 and 14). Previously, execution status was OR-ed across reports independently for each side, so a test covered by one report and an implementation covered by a different report passed. If a previously-green run fails after this change, the per-witness diagnostics name each bound witness and its result; the usual fixes are producing one coverage report per test (rather than one aggregate suite report) and moving annotations so each claim is unambiguous.
