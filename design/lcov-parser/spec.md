...
Rationale for the near-miss rejections: `SF`, `DA`, and `end_of_record`
are the three record types the parser's output and block structure depend
on, so a near-miss of one of them is producer error or corruption, not a
future record type — no known LCOV record keyword extends `SF`, `DA`, or
`end_of_record`, and every surveyed producer emits the exact spelling (see
[decisions.md, Decision 9](decisions.md#decision-9) and
[Decision 11](decisions.md#decision-11)). Ignoring such a line
...
either consume or reject; and an `SF` record missing its `:` would either
fold the next block's records into the previous file (when it appears
inside an open block) or parse the report as silently empty coverage (when
its block contains no `DA` records before end-of-input).
