...
impl Meta {
    fn set(&mut self, key: Option<Slice>, value: Slice) -> Result<()> {
        ...
        let prev = match key.as_deref() {
            ...
            Some(_) => {
                let key = key.unwrap();
                let err: Error = anyhow!("invalid metadata field {key:?}").into();
                let err = err.with_source_slice(key, "defined here");
                return Err(err);
            }
    ...
    fn build(self, contents: Vec<Slice>, default_type: AnnotationType) -> Result<Annotation> {
        ...
        let Some(target) = self.target else {
            let err: Error = anyhow!("comment is missing source specification").into();
            let err = err.with_source_slice(first_meta, "defined here");
            return Err(err);
        };
        ...
        for (allowed, field) in [
            ...
            if anno != allowed {
                if let Some(value) = field {
                    let err: Error = anyhow!("invalid key for type {anno}").into();
                    let err = err.with_source_slice(value.clone(), "defined here");
                    return Err(err);
                }
            }
        }
